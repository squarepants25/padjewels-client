  ...
  if (products && sortType && sortValue) {
    if (sortType === "category") {
      ...
      const filteredProducts = products.filter((product) => {
        console.log("Product Category ObjectId:", product);

        console.log(product.productCategories == sortValue);
        return product.productCategories == sortValue;
      });
    ...
    if (sortType === "search") {
      console.log("sortyoe", sortValue.toLowerCase());
      return products.filter((product) =>
        product.productName.toLowerCase().includes(sortValue.toLowerCase())
      );
    }
...
const fetchCategoryById = async (categoryId) => {
  try {
    const response = await axios.get(
      `${BASE_URL}api/v1/product/cat/${categoryId}`
    );
